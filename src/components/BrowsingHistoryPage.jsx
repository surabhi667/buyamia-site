...
export default function BrowsingHistoryPage() {
  ...
  async function load(page = 1) { setLoading(true); setMessage(''); try { const response = await fetch(`/api/account/history?page=${page}&limit=${pageSize}`); const payload = await response.json(); if (!response.ok) throw new Error(payload.error?.message || 'Unable to load your browsing history.'); setItems(payload.data); setMeta(payload.meta) } catch (error) { setMessage(error.message) } finally { setLoading(false) } }
  useEffect(() => { load() }, [])
  async function remove(id) { setMessage(''); try { const response = await fetch(`/api/account/history/${encodeURIComponent(id)}`, { method: 'DELETE' }); const payload = await response.json(); if (!response.ok) throw new Error(payload.error?.message || 'Unable to remove this item.'); if (items.length === 1 && meta.page > 1) await load(meta.page - 1); else { setItems((current) => current.filter((item) => item.id !== id)); setMeta((current) => ({ ...current, total: current.total - 1 })) } } catch (error) { setMessage(error.message) } }
  async function clear() { setMessage(''); try { const response = await fetch('/api/account/history', { method: 'DELETE' }); const payload = await response.json(); if (!response.ok) throw new Error(payload.error?.message || 'Unable to clear browsing history.'); await load() } catch (error) { setMessage(error.message) } }
