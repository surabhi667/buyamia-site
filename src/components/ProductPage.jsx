...
export default function ProductPage({ id }) {
  const [product, setProduct] = useState(null); const [image, setImage] = useState(0); const [quantity, setQuantity] = useState(''); const [note, setNote] = useState(''); const [tab, setTab] = useState('general'); const [contextOpen, setContextOpen] = useState(false); const [submitting, setSubmitting] = useState(false); const [adding, setAdding] = useState(false); const [saved, setSaved] = useState({ saved: false, item: null }); const [savingSaved, setSavingSaved] = useState(false); const [message, setMessage] = useState(''); const [error, setError] = useState(''); const [email, setEmail] = useState(''); const [subscribed, setSubscribed] = useState(false)
  useEffect(() => { const controller = new AbortController(); const query = new URLSearchParams(window.location.search); const productUrl = `/api/products/${encodeURIComponent(id)}${query.get('flashSale') ? `?flashSale=${encodeURIComponent(query.get('flashSale'))}` : ''}`; api(productUrl, { signal: controller.signal }).then((payload) => { setProduct(payload.data); setQuantity(String(payload.data.minimumOrder || 1)) }).catch((caught) => { if (caught.name !== 'AbortError') setError(caught.message) }); return () => controller.abort() }, [id])
  useEffect(() => { const controller = new AbortController(); fetch(`/api/saved/status?productId=${encodeURIComponent(id)}`, { credentials: 'include', signal: controller.signal }).then(async (response) => { const payload = await response.json(); if (response.status === 401) return; if (!response.ok) throw new Error(payload.error?.message || 'Unable to load saved status.'); setSaved(payload.data) }).catch((caught) => { if (caught.name !== 'AbortError') setMessage(caught.message) }); return () => controller.abort() }, [id])
  ...
  async function subscribe(event) { event.preventDefault(); try { await api('/api/newsletter', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ email, source: 'product-page' }) }); setSubscribed(true) } catch (caught) { setMessage(caught.message) } }
  async function addToCart() {
    ...
    try {
      const response = await fetch('/api/cart/items', {
        ...
        body: JSON.stringify({ productId: product.id, quantity: Math.max(1, selected / minimumOrder), packSize: minimumOrder, ...(product.flashSale ? { flashSaleId: product.flashSale.id } : {}) }),
      })
  ...
  async function toggleSaved() {
    ...
    const nextSaved = !saved.saved
    setSavingSaved(true)
    setSaved({ saved: nextSaved, item: nextSaved ? saved.item : null })
    setMessage(nextSaved ? 'Saved.' : 'Removed from Saved.')
    try {
      const response = await fetch(nextSaved ? '/api/saved' : `/api/saved/products/${encodeURIComponent(product.id)}`, {
        method: nextSaved ? 'POST' : 'DELETE',
        credentials: 'include',
        headers: nextSaved ? { 'Content-Type': 'application/json' } : {},
        ...(nextSaved ? { body: JSON.stringify({ productId: product.id }) } : {}),
      })
      const payload = await response.json()
      if (response.status === 401) {
        setSaved(previous)
        requestAuth()
        throw new Error('Please log in or sign up to save this product.')
      }
      if (!response.ok) throw new Error(payload.error?.message || 'Unable to update Saved.')
      setSaved(nextSaved ? { saved: true, item: payload.data } : { saved: false, item: null })
      window.dispatchEvent(new CustomEvent('buyamia:saved-updated', { detail: { productId: product.id, saved: nextSaved } }))
    } catch (caught) {
      setSaved(previous)
      setMessage(caught.message)
    } finally {
